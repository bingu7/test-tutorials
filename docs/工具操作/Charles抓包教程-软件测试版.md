...
## 动手任务：用弱网模拟复现「移动端提交订单后白屏」

> 这是本教程的收尾练习。请**独立完成**，不要先看参考答案。目标不是"会开弱网"，而是**能用弱网+抓包定位移动端的时序缺陷**。

### 任务背景

测试反馈："App 在 4G 信号差的时候，点提交订单会白屏，在家里的 WiFi 下从没出现过。"

开发说："本地连的 WiFi，复现不了。"

这是一个典型的**只在弱网下暴露的时序问题**。你要用 Charles 的弱网模拟把它稳定复现出来。

### 任务准备

1. 用 Charles 配置好手机代理（参考教程"五、移动端抓包配置"）。
2. 打开 **Proxy → Throttle Settings**，启用弱网。
3. 建议参数（模拟较差的 4G）：

| 参数 | 值 | 含义 |
|------|-----|------|
| Bandwidth | 256 kbps | 带宽上限 |
| Utilisation | 50% | 可用带宽占比 |
| Round-trip Latency | 800 ms | 往返延迟 |
| MTU | 1500 | 最大传输单元 |
| Reliability | 80% | 丢包率 20% |

### 任务要求

请依次完成，并**保留抓包截图/会话记录**：

1. **稳定复现**：在弱网下重复提交订单至少 5 次，记录白屏出现的次数，说明是否稳定复现。
2. **定位超时点**：找出白屏发生时，**哪个请求耗时异常**（给出接口路径和耗时）。
3. **判断前后端行为**：白屏时，前端有没有拿到响应？拿到的是什么（超时/错误码/空数据）？
4. **区分「弱网导致」和「代码缺陷」**：说明为什么这**仍然是缺陷**，而不是"网差活该"。
5. **给出可复用产出**：写出一份"移动端弱网验收清单"（至少 4 条），可以在每个版本发布前执行。

### 提交物

| 产出 | 要求 |
|------|------|
| 复现记录 | 5 次提交的白屏次数与对应接口耗时 |
| 关键会话 | 白屏时的请求/响应截图 |
| 验收清单 | 第 5 题的清单，可直接用于回归 |
| 结论 | 用 3-5 句话说明：根因在哪、是前端还是后端、建议怎么修 |

### 完成标准

- [ ] 能在弱网下**稳定复现**（或明确说明复现概率），不是"试了一次没出现"
- [ ] 能指出具体的超时接口和耗时数字，不是笼统说"网络慢"
- [ ] 能解释"网络差"为什么**不构成免责理由**（需要有超时处理与用户提示）
- [ ] 验收清单里的每一条都能在 Charles 里实际配置并执行

??? tip "参考答案与思路（先自己做完再看）"

    **第 1 题：稳定复现**

    在 800ms 延迟 + 20% 丢包下，重复提交 5 次，通常能复现 3-5 次——**属于高概率复现**，可以判定为必现级缺陷（弱网条件下）。
    ...
    **第 2 题：定位超时点**
    ...
    关键接口通常是 **提交订单** `POST /api/order/submit`：

    - 正常网络：约 300-800ms
    - 弱网下：**超过客户端超时阈值**（很多 App 设 10s 或 30s），最终超时

    注意观察抓包里的标记：Charles 中该会话会显示为 **Failed / Timed out**，而不是收到响应。
    ...
    **第 3 题：前端拿到什么**
    ...
    ```text
    情况 A：请求发出后，服务端最终返回了 200，但前端已超时放弃
      → 抓包能看到完整响应，但前端 UI 已进入白屏
      → 说明前端超时阈值 < 服务端处理时间

    情况 B：请求根本没收到响应（连接被中断/超时）
      → 抓包显示 Failed
      → 说明是网络层超时
    ```

    无论哪种，**前端都没有正确处理失败分支** —— 白屏说明它既没展示"提交失败，请重试"，也没回退到可操作页面。

    **第 4 题：为什么仍是缺陷**

    这是**典型的时序缺陷**，而非"网络的错"：

    1. **没有超时兜底**：请求失败后应该给用户明确提示并允许重试，白屏等于功能不可用。
    2. **可能造成重复下单**：用户看到白屏会再点一次，如果后端没有幂等控制，会生成两笔订单——**这是更严重的次生风险**。
    3. **状态不确定**：用户不知道订单到底提交成功没有，属于严重体验与业务缺陷。

    > 判定标准：**任何网络条件下，App 都不应该白屏**。弱网只是暴露问题的条件，不是问题的原因。

    验证建议：在弱网下提交后立刻恢复网络，检查**是否真的生成了订单**，以确认幂等性是否缺失。

    **第 5 题：移动端弱网验收清单**

    ```text
    [ ] 1. 慢速网络（延迟 800ms / 带宽 256kbps）：核心流程（登录、下单、支付）无白屏、无卡死
    [ ] 2. 高丢包（Reliability 80%）：请求失败时有明确提示，且可重试成功
    [ ] 3. 网络中断后恢复：提交类请求不产生重复数据（幂等验证）
    [ ] 4. 超时阈值合理性：客户端超时时间 ≥ 服务端 P99 响应时间
    [ ] 5. 弱网下大资源加载：图片/列表有占位或骨架屏，不出现长时间空白
    ```

    执行方式：在 Charles 的 Throttle 预设里保存以上参数为 `Weak4G`，每个版本回归时一键切换执行。
...
完成教程后，建议做 [Charles 抓包测验](Charles抓包测验.md) 检验学习效果。
