...
| `lsof -i:8080` | 显示占用 8080 端口的进程 |
| `grep 8080` | 过滤 8080 端口 |
| `findstr :8080` | Windows 下过滤 8080 端口 |
| `tasklist` | Windows 下显示所有进程 |
...
| TTL 设置 | 效果 |
|---------|------|
| `TTL = 300`（5 分钟） | 改了 DNS 记录，最多 5 分钟后全球生效 |
| `TTL = 86400`（24 小时） | 改了 DNS 记录，最多要等 24 小时才全球生效 |
...
> **这条命令的用途：** 绕过 DNS，直接用 IP 访问服务器。`-H "Host: ..."` 手动指定 Host 头，让服务器以为你在访问域名（虚拟主机场景必需）。
...
| 参数 | 含义 |
|------|------|
| `-c 4` | 发送 4 个包后停止（count） |
| `-t` | 持续发送，直到手动停止（Ctrl+C） |
| `-i 1` | 间隔 1 秒发送一次（Linux，默认 1 秒） |
| `-s 64` | 发送 64 字节的数据包 |
| `-W 2` | 等待 2 秒超时（Linux） |
...
| 参数 | 含义 |
|------|------|
| `-o /dev/null` | 丢弃响应体内容（`/dev/null` 是 Linux 的"黑洞"，写了就消失），只关心耗时 |
| `-s` | 隐藏进度条（适合写脚本时用） |
| `-w "..."` | 自定义输出格式，`%{time_total}` 等是 curl 内置变量 |
...
| `-b cookie.txt` | 从文件读取 Cookie |

> **Windows 注意：** `/dev/null` 改为 `NUL`，反斜杠 `\` 换行改为 `^`。
...
## 动手任务：从「页面打不开」反推网络故障层

> 这是本教程的收尾练习。请**独立完成**，不要先看参考答案。目标不是"背出排障命令"，而是**能根据现象判断故障在哪一层**。

### 任务背景

测试同学反馈："测试环境页面打不开，有时候能打开有时候不能。"开发说"我本地是好的"。你需要用**分层排查法**定位，而不是让运维重启一遍试试。

### 任务数据

以下是四位同事在同一时刻的排障输出（`app.test.internal` 是测试环境域名）：

```text
同事 A：
$ ping app.test.internal
PING app.test.internal (10.20.30.41) 56(84) bytes of data.
--- app.test.internal ping statistics ---
10 packets transmitted, 0 received, 100% packet loss

同事 B：
$ nslookup app.test.internal
Server:   10.20.30.2
Address:  10.20.30.2#53
Name:     app.test.internal
Address:  10.20.30.41

同事 C：
$ curl -v http://app.test.internal/api/health
*   Trying 10.20.30.41:80...
* Connected to app.test.internal (10.20.30.41) port 80 (#0)
> GET /api/health HTTP/1.1
> Host: app.test.internal
...
a1b2c3d4e5f6   myapp:1.0    Up 3 minutes (unhealthy)   0.0.0.0:8080->8080/tcp
```

另有一条关键信息：同事 D 的容器是 3 分钟前**刚重启过**（这次重启是因为"内存超限被 OOM Kill"）。

### 任务要求

请依次完成：

1. **判断故障层**：A 的 `ping` 100% 丢包，说明是网络不通吗？如果不是，怎么解释？
2. **解释 502 的来源**：同事 C 明明"Connected"成功了，为什么返回 502？502 是哪一层的问题？
3. **找出根因链**：把 A/B/C/D 四条输出串成一条因果链，说明"有时能打开有时不能"的原因。
4. **验证你的判断**：给出 2-3 条能证实（或证伪）你结论的命令。
5. **给出修复与预防**：这次怎么恢复？下次怎么让问题在测试前就暴露？

### 提交物

| 产出 | 要求 |
|------|------|
| 分层判断 | 逐条说明 A/B/C/D 分别证明了什么、不能证明什么 |
| 因果链 | 用箭头串起从现象到根因的推导 |
| 验证命令 | 第 4 题的命令 + 预期输出 |
| 结论 | 用 3-5 句话说明：是网络问题、配置问题还是应用问题，责任方是谁，建议怎么预防 |

### 完成标准

- [ ] 能说明 `ping` 不通**不等于**服务不可达（很多环境禁 ICMP 或有防火墙策略）
- [ ] 能准确区分 502 / 503 / 504 各自代表什么、分别由谁产生
- [ ] 因果链完整，每一环都有对应的命令证据
- [ ] 第 5 题的预防措施能真正落到测试流程里（不是"让开发注意一下"）

??? tip "参考答案与思路（先自己做完再看）"

    **第 1 题：A 的 ping 丢包说明不了太多**

    `ping` 走的是 **ICMP**，而 Web 服务走 **TCP**。很多生产/准生产环境出于安全考虑**直接封禁 ICMP**，或者中间有防火墙策略只放行 80/443。

    关键证据：同事 C 的 `curl` **成功建立了 TCP 连接**（`Connected to ... port 80`）。这说明**网络是通的**——如果网络真不通，`curl` 会在 "Trying..." 后超时。所以：A 的结论（网络不通）**是错的**，B/C 的结论才是有效的。

    > 排障启示：不要用一层协议的现象去推断另一层。`ping` 不通时，先 `telnet host port` 或 `curl -v` 验证目标端口。

    **第 2 题：502 是网关层问题**

    502 Bad Gateway 的语义是：**反向代理（这里是 nginx）成功收到了请求，但把请求转发给上游（应用容器）时，上游没有给出有效响应**。

    - 谁产生 502？是 **nginx**，不是应用自己。
    - 说明什么？nginx 是活的、能接受连接，但**后端应用可能没起来/刚崩溃/端口没监听**。

    对比记忆：

    | 状态码 | 含义 | 通常由谁产生 |
    |--------|------|--------------|
    | 502 Bad Gateway | 上游返回无效响应/无法连接 | 网关（nginx） |
    | 503 Service Unavailable | 服务暂不可用（过载/维护/无健康实例） | 网关或应用 |
    | 504 Gateway Timeout | 上游处理超时 | 网关（nginx） |

    **第 3 题：因果链**

    ```
    应用容器内存超限 → 被 OOM Kill → 容器重启（Up 3 minutes）
        → 重启后应用还在启动中/启动失败（unhealthy）
        → nginx 把请求转发给该容器，拿不到有效响应
        → 返回 502 Bad Gateway
        → 用户侧表现为"有时能打开"（某个副本还健在时能通，全挂时全 502）
    ```

    "有时能打开有时不能"的解释：如果有**多个应用副本**，nginx 轮询时打到健康副本就成功、打到正在重启的副本就 502——这正好解释了"时好时坏"。

    同事 D 的 `(unhealthy)` 是最关键的证据：容器进程活着（Up），但**健康检查不通过**，说明应用没真正就绪。

    **第 4 题：验证命令**

    ```bash
    # 1. 看应用容器日志，确认是不是 OOM / 启动报错
    docker logs a1b2c3d4e5f6 --tail 50
    # 关注：Killed、OOM、以及启动异常

    # 2. 确认应用端口是否真的在监听（在容器内执行）
    docker exec a1b2c3d4e5f6 netstat -tlnp | grep 8080
    # 若无输出 → 应用根本没起来，502 得到证实

    # 3. 绕过 nginx 直连应用，判断问题在网关还是应用
    curl -v http://127.0.0.1:8080/api/health
    # 直连也失败 → 应用问题；直连成功但经 nginx 失败 → 网关配置问题

    # 4. 查看容器重启历史与退出码（区分 OOM 与普通崩溃）
    docker inspect -f '{{.State.OOMKilled}} {{.State.ExitCode}} {{.RestartCount}}' a1b2c3d4e5f6
    ```

    第 3 条命令是**定位分层的关键**：直连成功与否，直接决定责任方是应用还是网关。
    ...
    **第 5 题：修复与预防**
    ...
    ```bash
    # 先确认是否为内存限制过低导致，再重启
    docker stats --no-stream a1b2c3d4e5f6
    docker restart a1b2c3d4e5f6
    ```

    根本预防（落到测试流程）：
    1. **把健康检查接入部署门禁**：容器 `unhealthy` 时不应被 nginx 摘除前就开始接收流量，部署脚本要等 `healthy` 再切流。
    2. **回归用例里加一条"依赖健康"断言**：在接口自动化里先请求 `/api/health`，不健康直接 fail——避免后续用例因环境问题产生大量假失败。
    3. **内存限制纳入环境配置评审**：OOM Kill 说明 limit 设得低于实际用量，属于**环境配置缺陷**，应作为缺陷记录而不是"重启一下就好"。
    4. **监控告警**：容器重启次数（`RestartCount`）超过阈值应告警，而不是等用户反馈。
...
=== "Linux / Mac"
    ...
    curl -v url                  # HTTP 调试
    ss -tlnp                     # 查看监听端口
    tcpdump -i any port 8080     # 抓包（-i any 监听所有网卡，port 8080 只抓 8080 端口的包）
    ```
...
3. **如果你想检验网络知识**：做 [网络知识测验](网络知识测验.md)，检验 HTTP、状态码、Cookie/Token 的掌握程度
