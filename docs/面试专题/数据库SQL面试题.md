...
```text
存储过程 = 预编译的 SQL 语句集合，存在数据库中，应用程序直接调用。
...
```text
第一范式（1NF）：字段具有原子性，不能再拆分
  错误：一个字段存 "张三,13800138000"
  正确：拆成 name 和 phone 两个字段

第二范式（2NF）：满足 1NF，非主键字段必须完全依赖主键
  错误：订单表里存商品名称（只依赖商品ID，不依赖订单ID）
  正确：商品名称放商品表，订单表只存商品ID

第三范式（3NF）：满足 2NF，非主键字段不能互相依赖
  错误：学生表里存学院名称和学院电话（学院电话依赖学院名称，不直接依赖学生ID）
  正确：学院信息单独建表，学生表只存学院ID

简单理解：字段拆清楚、数据不重复、表关系合理。
```
...
理解执行顺序对写 SQL 有帮助：
- WHERE 里不能用 SELECT 的别名（WHERE 先执行）
- ORDER BY 可以用 SELECT 的别名（ORDER BY 后执行）
- GROUP BY 里的字段必须出现在 SELECT 中（或用聚合函数）
```
