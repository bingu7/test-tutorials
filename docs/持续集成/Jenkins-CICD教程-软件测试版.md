...
先跑通最小闭环，再学习参数化构建、定时任务、Allure 报告、通知和质量门禁。

### 版本与维护说明

| 项目 | 说明 |
|------|------|
| 适用工具 | Jenkins LTS、Pipeline、常见测试报告插件 |
| 使用建议 | 先用最小 Pipeline 跑通测试，再加凭据、报告、通知和权限 |
| 更新提醒 | Jenkins 对 JDK、插件和安全配置要求会变化，安装或升级前先查看当前 LTS 要求 |
...
1. **如果你想做项目实战**：进入 [CI/CD 自动化回归实战](../项目实战/CICD自动化回归实战/)，把自动化接入流水线
2. **如果你想巩固容器化**：学习 [Docker 容器](../工具操作/Docker容器教程-软件测试版/)，掌握环境搭建
3. **如果你想做完整项目**：进入 [项目实战](../项目实战/)，完成完整测试流程

### 通关检查

完成本阶段后，使用 [第5阶段-项目面试通关](../学习中心/第5阶段-项目面试通关/) 检查是否可以进入下一阶段。
